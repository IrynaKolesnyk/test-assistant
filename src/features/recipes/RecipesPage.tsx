      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-[var(--bg-panel)] border border-[var(--border)] rounded-2xl p-5">
            <h3 className="text-sm font-semibold text-[var(--text-1)] mb-3">🧂 {t('recipes.ingredients')}</h3>
          ...
          <div className="md:col-span-2 bg-[var(--bg-panel)] border border-[var(--border)] rounded-2xl p-5">
            <h3 className="text-sm font-semibold text-[var(--text-1)] mb-3">📋 {t('recipes.instructions')}</h3>
    ...
    if (meals.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center py-16 text-[var(--text-3)]">
          ...
          <p className="text-sm">{t('recipes.noResults')}</p>
        </div>
      )
    }
    ...
    return (
      <div className="bg-[var(--bg-panel)] border border-[var(--border)] rounded-2xl p-5">
        ...
        <div className="text-sm text-[var(--text-1)] leading-relaxed whitespace-pre-wrap">
          {aiRecipe || <span className="text-[var(--text-3)]">{t('recipes.searching')}</span>}
        </div>
      </div>
    )
  }
    ...
    <div className="flex flex-col h-full bg-[var(--bg-base)]">
      <header className="flex-shrink-0 border-b border-[var(--border)] bg-[var(--bg-panel)] px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-[var(--text-1)]">{t('recipes.title')}</h2>
        </div>
        <span className="text-xs text-[var(--text-3)]">
          {usingFreeApi ? t('recipes.freeApi') : model}
        </span>
      </header>

      {usingFreeApi && (
        <div className="flex-shrink-0 bg-blue-500/10 border-b border-blue-500/20 px-6 py-3 text-sm text-blue-400">
          {t('recipes.freeBanner', { interpolation: { escapeValue: false } }).replace('<bold>', '').replace('</bold>', '')}{' '}
          <a href="/settings" className="underline hover:text-blue-300">
            {t('recipes.addKeyLink')}
          </a>{' '}
          {t('recipes.forAiRecipes')}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-5">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            {query && (
              <button
                onClick={clearAll}
                aria-label={t('recipes.clearInput')}
        ...
        {showEmpty && (
          <div className="text-center text-sm text-[var(--text-3)] py-8">{t('recipes.noResults')}</div>
        )}
